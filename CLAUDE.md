...
- **Federated React config panel**: `src/configpanel/PluginConfigurationPanel.tsx` is the composition root, with domain-specific components and hooks under `src/configpanel/`. `signalk-nearlcrews-ui` 0.6.2 provides the panel root, theme persistence, layouts, fields, feedback, metrics, collapsible sections, and action bar. Project CSS stays in focused CSS modules and consumes public `--snui-*` tokens. webpack bundles the shared UI library but consumes React from the host as a singleton with `requiredVersion: '>=19.2.0 <20.0.0'` and `import: false`. The exposed panel stays in one async chunk to avoid duplicate compression overhead. `npm run check:panel` verifies the built remote, CSS, host-shared React, library marker, and bundle budget. The JSON `schema()` remains the fallback for older admin UIs.
...
- React 19.2, `signalk-nearlcrews-ui` 0.6.2, webpack 5, esbuild-loader 4.5,
