...
- **PGNs** (when paired with `signalk-nmea2000-emitter-cannon`): 130311 (pressure), 130312 (temperatures via fixed enum slots: temperature, dewPoint, apparentWindChill, heatIndex), 130313 (relativeHumidity), 130306 (wind: `speedOverGround`, `directionTrue` only). The plugin's apparent wind (`environment.weather.windSpeedApparent` / `windAngleApparent`) and gust (`environment.weather.speedGust`) are producer-namespaced and do NOT bridge to NMEA2000: the cannon's 130306 wind conversion subscribes to the canonical `environment.wind.speedApparent` / `angleApparent` leaves, which this plugin deliberately no longer emits. Synthetic apparent wind reaches Signal K consumers only; a real masthead anemometer should feed PGN 130306 apparent wind. Instance numbers and bus priority are assigned by the companion plugin, not embedded in the deltas this plugin produces.
...
- Vitest 4.1+ for testing (272 tests across 11 files; mutation score 67% via Stryker.js, opt-in via `npm run mutation-test`)
