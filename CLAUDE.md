...
- **Weather API provider**: `index.ts` registers a Signal K v2 Weather API provider via `app.registerWeatherProvider(...)` in `startServices`, and unregisters via `app.weatherApi.unRegister(PLUGIN.NAME)` in the `stop` closure. `WeatherProviderAdapter` (`src/services/WeatherProviderAdapter.ts`) implements the provider; `getForecasts('point')` is backed by the AccuWeather 12-hour hourly endpoint and `getForecasts('daily')` by the 5-day daily endpoint, and `getObservations` by the AccuWeather current-conditions endpoint (via `AccuWeatherService.getCurrentConditionsForLocation`, honoring the caller-supplied position, not the vessel position), all mapped to the SI `WeatherData` envelope by pure functions in `src/mappers/WeatherProviderMapper.ts` (`mapCurrentToObservation` adds pressure and pressure tendency the forecasts lack). The v2 provider is registered only when AccuWeather is the active source, since the forecast and observation endpoints are AccuWeather-backed; under Open-Meteo the emission path still works but the v2 API is not advertised. `getWarnings` is served by the keyless, region-aware `WarningsService` (`src/services/WarningsService.ts`): NWS CAP active alerts for US waters (identifying User-Agent, best-effort so an outage or uncovered point returns an empty list), an empty list elsewhere, mapped to the `WeatherWarning` shape by `mapNwsAlertsToWarnings` in `src/mappers/WarningsMapper.ts`. Because warnings ride the v2 provider, they are currently exposed only while AccuWeather is active; Met.no MetAlerts for Nordic waters is the planned second region. Forecast and observation fetches share the one `AccuWeatherService` instance, its location-key cache, and its rolling-24h quota window; an on-demand cache (`FORECAST_CACHE` TTLs: 30 min hourly, 3 h daily, 10 min observations) plus stale-on-quota-exhaustion keeps a polling consumer from exhausting the key. Registering the provider is what makes the server advertise `weather` in `/signalk/v2/features`, which is the flag dashboards like signalk-binnacle gate their weather UI on.
...
- Vitest 4.1+ for testing (418 tests across 23 files; mutation score 67% via Stryker.js, opt-in via `npm run mutation-test`). `npm test` runs once (registry/CI safe); `npm run test:watch` is the interactive watcher.
