...
- Fixed status banner stuck at `Running, awaiting first update` after the first successful fetch. Previously the banner was pushed to `setPluginStatus` once during `finalizePluginStart` (when `lastUpdate` was still null) and never refreshed except on stale-error recovery, so weather data could be flowing while the admin UI showed the start-time string indefinitely. `emitWeatherTick` in `index.ts` now re-pushes `formatStatusBanner()` on every fresh tick, which also subsumes the old `staleErrorActive` recovery flag flip. New regression test in `src/__tests__/index.test.ts` ("refreshes the status banner on emission ticks after weather data is available", 235 total tests). Bundle still 66.2 KB.
