...
This patch release restores the configuration panel that version 1.13.4 left
unable to load on Signal K 2.24.x hosts, fixes two configuration-panel defects
that discarded work when a section was collapsed, reports the paused and stale
states in the panel's status banner, keeps the merge list from silently
restoring every provider, corrects a Met.no observation that reported a
forecast value, and refreshes the shared panel. No configuration migration is
required.

### Fixed

- The configuration panel loads again on Signal K 2.24.x hosts, where the
  1.13.4 panel did not mount at all. The Admin UI registers its React share as
  19.0.0 while actually shipping React 19.2.4, so the strict share check
  1.13.4 introduced rejected a fully compatible host. The host-provided
  singletons keep their `^19.2.0` requirement, but a version mismatch now
  warns and continues instead of refusing to mount, and the packaging gate
  rejects a reintroduced strict check.
