...
export class AccuWeatherService {
	...
	private locationCache = new Map<
		string,
		{ location: AccuWeatherLocation; timestamp: number }
	>();
	...
	private transformWeatherData(
		...
		const temperature =
			conditions.Temperature.Metric.Value + UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
		const pressure =
			conditions.Pressure.Metric.Value * UNITS.PRESSURE.MILLIBAR_TO_PASCAL;
		const humidity = conditions.RelativeHumidity; // Keep as percentage (0-100) for Garmin compatibility
		const windSpeed =
			conditions.Wind.Speed.Metric.Value * UNITS.WIND_SPEED.KMH_TO_MS;
		const windDirection =
			conditions.Wind.Direction.Degrees * UNITS.ANGLE.DEGREES_TO_RADIANS;
		const dewPoint =
			conditions.DewPoint.Metric.Value + UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
		const windChill =
			conditions.WindChillTemperature.Metric.Value +
			UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
		const heatIndex =
		...
		const realFeelShade =
			conditions.RealFeelTemperatureShade.Metric.Value +
			UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
		const wetBulbTemperature =
			conditions.WetBulbTemperature.Metric.Value +
			UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
		...
		const windGustSpeed =
			conditions.WindGust.Speed.Metric.Value * UNITS.WIND_SPEED.KMH_TO_MS;
		...
		const precipitationCurrent =
			conditions.PrecipitationSummary.PastHour.Metric.Value; // mm in last hour

		// Temperature trends (new)
		const temperatureDeparture24h =
			conditions.Past24HourTemperatureDeparture.Metric.Value; // Keep as Celsius delta
		...
		const absoluteHumidityValue = calculateAbsoluteHumidity(
			temperature,
			humidity / 100,
		);
		const airDensityEnhanced = calculateAirDensity(
			temperature,
			pressure,
			humidity / 100,
		);
		const heatStressIndex = this.calculateHeatStressIndex(
	...
	private calculateHeatStressIndex(wetBulbGlobeTemperatureK: number): number {
		try {
			...
			const wbgtC =
				wetBulbGlobeTemperatureK - UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
	...
	private async getLocationKey(location: GeoLocation): Promise<string> {
		...
		if (
			cached &&
			Date.now() - cached.timestamp < this.config.locationCacheTimeout * 1000
		) {
		...
		if (this.locationCache.size >= AccuWeatherService.MAX_CACHE_SIZE) {
	...
	private async searchLocation(
		...
		const url = new URL(
			`${ACCUWEATHER.BASE_URL}${ACCUWEATHER.ENDPOINTS.LOCATION_SEARCH}`,
		);
	...
	private async getCurrentConditions(
		...
		const response =
			await this.makeApiRequest<AccuWeatherCurrentConditions[]>(url);
	...
	private async handleApiError(
		...
		try {
			...
			switch (statusCode) {
				case 401:
					throw new Error(
						`${ERROR_CODES.NETWORK.API_UNAUTHORIZED}: Invalid API key - ${message}`,
					);
				case 403:
					throw new Error(
				...
				case 503:
					...
					throw new Error(
						`${ERROR_CODES.NETWORK.NETWORK_ERROR}: Service unavailable - ${message}`,
					);
				default:
					throw new Error(
	...
	private validateLocation(location: GeoLocation): void {
		if (!location || typeof location !== "object") {
			throw new Error(
				`${ERROR_CODES.CONFIGURATION.INVALID_COORDINATES}: Invalid location object`,
			);
		}
	...
	private validateWeatherData(data: WeatherData): void {
		if (
			...
			this.logger("warn", "Temperature outside expected range", {
		...
		if (
			...
			this.logger("warn", "Pressure outside expected range", {
		...
		if (
			...
			this.logger("warn", "Humidity outside expected range", {
		...
		if (
			...
			this.logger("warn", "Wind speed outside expected range", {
