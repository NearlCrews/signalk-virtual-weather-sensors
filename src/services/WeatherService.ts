...
export class WeatherService {
	...
	public async start(): Promise<void> {
		...
		try {
			...
			this.app.setPluginError(
				`Weather service startup failed: ${errorMessage}`,
			);

			throw new Error(
				`${ERROR_CODES.SYSTEM.PLUGIN_START_FAILED}: ${errorMessage}`,
			);
		}
	}
	...
	public async stop(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`${ERROR_CODES.SYSTEM.PLUGIN_STOP_FAILED}: ${errorMessage}`,
			);
		}
	}
	...
	/**
	 * Fetch weather data for a specific position
	 * Used by the weather provider adapter
	 */
	public async fetchWeatherForPosition(position: {
		latitude: number;
		longitude: number;
		...
		try {
			...
			return await this.accuWeatherService.fetchCurrentWeather(geoLocation);
		} catch (error) {
			this.logger("error", "Failed to fetch weather for position", {
				error: error instanceof Error ? error.message : String(error),
			});
			return null;
		}
	...
	private async updateWeatherData(): Promise<void> {
		...
		try {
			...
			const weatherData =
				await this.accuWeatherService.fetchCurrentWeather(position);

			// Get current vessel navigation data
			const vesselData = this.signalKService.getVesselNavigationData();

			// Calculate enhanced weather values
			const enhancedWeatherData = await this.enhanceWeatherData(
				weatherData,
				vesselData,
			);
			...
			this.logger("info", "Weather data updated successfully", {
				...
				hasApparentWind: !!(
					enhancedWeatherData.apparentWindSpeed &&
					enhancedWeatherData.apparentWindAngle
				),
	...
	private async enhanceWeatherData(
		...
		const apparentWind = this.calculateApparentWindData(
			weatherData,
			vesselData,
		);
	...
	private calculateApparentWindData(
		...
		if (
			...
			return this.calculateApparentWindWithCompleteData(
				weatherData,
				vesselData,
			);
		}
	...
	private calculateApparentWindFallback(
		...
		if (!vesselData.isComplete) {
			this.logger(
				"debug",
				"Cannot calculate apparent wind - incomplete vessel data",
				{
	...
	private calculateApparentWindAngleFromHeading(
		...
		const vesselHeading =
			vesselData.headingTrue || vesselData.courseOverGroundTrue;
	...
	private emitWeatherData(weatherData: WeatherData): void {
		try {
			const vesselData = this.signalKService.getVesselNavigationData();
			const delta = this.createSignalKDelta(weatherData, vesselData);

			this.app.handleMessage(
				"signalk-virtual-weather-sensors",
				delta as Partial<Delta>,
			);
	...
	private createSignalKDelta(
		...
		this.addWindData(values, weatherData, vesselData);
	...
	private addEnhancedTemperatureData(
		...
		const temperatureFields = [
			...
			{
				data: weatherData.realFeelShade,
	...
	private addWindData(
		...
		this.addApparentWindAngleData(values, weatherData, vesselData);
		this.addMagneticWindDirection(values, weatherData, vesselData);
	}
	...
	private addApparentWindAngleData(
		...
		if (vesselHeading !== undefined && vesselHeading !== null) {
			const apparentWindDirection =
				this.windCalculator.calculateWindDirectionHeading(
					vesselHeading,
	...
	private addMagneticWindDirection(
		...
		const magneticVariation = vesselData.magneticVariation;

		if (magneticVariation !== undefined && magneticVariation !== null) {
			const directionMagnetic =
				this.windCalculator.calculateWindDirectionMagnetic(
					weatherData.windDirection,
					magneticVariation,
				);
			values.push({
	...
	private addAtmosphericData(
		...
		const atmosphericFields = [
			...
			{
				data: weatherData.airDensityEnhanced,
