...
export function convertAccuWeatherTemperature(
	...
	if (
		tempData?.Metric?.Value === undefined ||
		tempData?.Metric?.Value === null ||
		!Number.isFinite(tempData.Metric.Value)
	) {
		return null;
	}
...
export function convertAccuWeatherPressure(
	...
	if (
		pressureData?.Metric?.Value === undefined ||
		pressureData?.Metric?.Value === null ||
		!Number.isFinite(pressureData.Metric.Value)
	) {
		return null;
	}
 ...
 */
export function convertAccuWeatherHumidity(
...
export function convertAccuWeatherWindSpeed(
	...
	if (
		windSpeedData?.Metric?.Value === undefined ||
		windSpeedData?.Metric?.Value === null ||
		!Number.isFinite(windSpeedData.Metric.Value)
	) {
		return null;
	}
...
export function convertAccuWeatherWindDirection(
	...
	if (
		windDirData?.Degrees === undefined ||
		windDirData?.Degrees === null ||
		!Number.isFinite(windDirData.Degrees)
	) {
		return null;
	}
...
export function isValidPressure(pressure: number): boolean {
	return isWithinBounds(
		pressure,
		VALIDATION_LIMITS.PRESSURE.MIN,
		VALIDATION_LIMITS.PRESSURE.MAX,
	);
}
 ...
 */
export function sanitizeWeatherData<T extends Record<string, unknown>>(
	data: T,
): T {
...
export function calculateAirDensity(
	...
	const density =
		dryAirPressure / (R_d * temperatureK) +
		vaporPressure / (R_v * temperatureK);
