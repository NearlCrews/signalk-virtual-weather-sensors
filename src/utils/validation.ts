...
function validateEnhancedFields(
	...
	if (
		...
		(data.visibility < 0 || data.visibility > 50000)
	) {
		warnings.push(
			`Visibility ${data.visibility}m is outside typical range (0-50000m)`,
		);
	}

	if (
		data.cloudCover !== undefined &&
		(data.cloudCover < 0 || data.cloudCover > 1)
	) {
	...
	if (
		...
		(data.beaufortScale < 0 || data.beaufortScale > 12)
	) {
		warnings.push(
			`Beaufort scale ${data.beaufortScale} is outside valid range (0-12)`,
		);
	}
}
 ...
 */
export function validateTemperatureConsistency(
	data: Partial<WeatherData>,
): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
	...
	if (data.temperature && data.windChill) {
		if (
			data.windChill > data.temperature &&
			data.windSpeed &&
			data.windSpeed > 1
		) {
			warnings.push(
				"Wind chill is higher than air temperature despite wind presence",
			);
		}
	}
 ...
 */
function validatePositionFields(
	...
	if (!isValidLatitude(data.position.latitude)) {
		errors.push(
			`Invalid latitude: ${data.position.latitude} (must be between -90 and 90)`,
		);
	}
	if (!isValidLongitude(data.position.longitude)) {
		errors.push(
			`Invalid longitude: ${data.position.longitude} (must be between -180 and 180)`,
		);
	}
}
...
function validateSpeedOverGround(
	...
	if (data.speedOverGround < 0) {
		...
		warnings.push(
			`Speed over ground ${data.speedOverGround}m/s seems unusually high (>100m/s)`,
		);
	}
}
...
function validateCourseOverGround(
	...
	if (
		data.courseOverGroundTrue < 0 ||
		data.courseOverGroundTrue > 2 * Math.PI
	) {
		warnings.push(
...
function validateDataAge(
	...
	if (data.dataAge > 60) {
		warnings.push(
			`Vessel data is ${data.dataAge} seconds old (consider refreshing)`,
		);
	}
	if (data.dataAge > 300) {
		errors.push(
			`Vessel data is too old (${data.dataAge} seconds) for reliable calculations`,
		);
	}
}
 ...
 */
export function isCompleteForWindCalculations(
	data: Partial<VesselNavigationData>,
): boolean {
	return !!(
		data.position &&
...
function validateApiKey(
	...
	if (
		...
		typeof config.accuWeatherApiKey !== "string"
	) {
	...
	} else if (config.accuWeatherApiKey.length < 10) {
		warnings.push("AccuWeather API key seems unusually short");
	}
}
...
function validateUpdateFrequency(
	...
	if (
		typeof config.updateFrequency !== "number" ||
		!Number.isFinite(config.updateFrequency)
	) {
...
function validateEmissionInterval(
	...
	if (
		typeof config.emissionInterval !== "number" ||
		!Number.isFinite(config.emissionInterval)
	) {
	...
	if (config.emissionInterval < 1) {
		...
		warnings.push(
			"Emission interval over 60 seconds may not be suitable for real-time NMEA2000",
		);
	}
}
 ...
 */
function validateResponseIsArray(
	response: unknown,
	errors: string[],
): response is unknown[] {
 ...
 */
function validateTemperatureStructure(
	data: Record<string, unknown>,
	errors: string[],
): void {
 ...
 */
function validateWindStructure(
	data: Record<string, unknown>,
	errors: string[],
): void {
 ...
 */
export function validateAccuWeatherResponse(
	response: unknown,
): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
 ...
 */
export function validateNMEA2000Ranges(
	data: Partial<WeatherData>,
): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
	...
	if (data.temperature !== undefined) {
		...
		if (tempC < -40 || tempC > 85) {
			warnings.push(
				`Temperature ${tempC.toFixed(1)}°C is outside NMEA2000 range (-40°C to +85°C)`,
			);
		}
	}
	...
	if (data.windSpeed !== undefined && data.windSpeed > 102.3) {
		warnings.push(
			`Wind speed ${data.windSpeed}m/s exceeds NMEA2000 maximum (102.3m/s)`,
		);
	}

	if (data.windGustSpeed !== undefined && data.windGustSpeed > 102.3) {
		warnings.push(
			`Wind gust speed ${data.windGustSpeed}m/s exceeds NMEA2000 maximum (102.3m/s)`,
		);
	}
	...
	if (
		...
		errors.push(
			`Humidity ${data.humidity} must be between 0 and 100 (percentage)`,
		);
	}
...
export function sanitizeForNMEA2000(data: WeatherData): WeatherData {
	...
	if (sanitized.windGustSpeed !== undefined) {
		sanitized.windGustSpeed = Math.max(
			0,
			Math.min(102.3, sanitized.windGustSpeed),
		);
	}
 ...
 */
export function validateCompleteWeatherData(
	data: Partial<WeatherData>,
): ValidationResult {
