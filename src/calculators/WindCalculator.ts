...
export class WindCalculator {
	...
	public calculateApparentWindSpeed(
		...
		try {
			...
			const apparentWindSpeed = Math.sqrt(
				apparentWindX ** 2 + apparentWindY ** 2,
			);

			// Validate result
			if (!Number.isFinite(apparentWindSpeed) || apparentWindSpeed < 0) {
				this.logger("warn", "Invalid apparent wind speed calculated", {
					apparentWindSpeed,
				});
	...
	public calculateWindChill(temperatureK: number, windSpeedMs: number): number {
		try {
			if (
				...
				this.logger("warn", "Invalid wind chill inputs", {
					temperatureK,
					windSpeedMs,
				});
				return 0;
	...
	public calculateHeatIndex(
		...
		try {
			if (
				...
				this.logger("warn", "Invalid heat index inputs", {
					temperatureK,
					relativeHumidity,
				});
				return temperatureK;
			...
			const tempF =
				((temperatureK - UNITS.TEMPERATURE.CELSIUS_TO_KELVIN) * 9) / 5 + 32;
			...
			const result =
				((heatIndex - 32) * 5) / 9 + UNITS.TEMPERATURE.CELSIUS_TO_KELVIN;
	...
	public calculateDewPoint(
		...
		try {
			if (
				...
				this.logger("warn", "Invalid dew point inputs", {
					temperatureK,
					relativeHumidity,
				});
			...
			if (!Number.isFinite(result) || result > temperatureK) {
				this.logger("warn", "Invalid dew point result", {
					dewPointC,
					result,
					temperatureK,
				});
	...
	public calculateBeaufortScale(windSpeed: number, gustSpeed?: number): number {
		try {
			...
			const effectiveWindSpeed = gustSpeed
	 ...
	 */
	public calculateRelativeWindDirection(
	 ...
	 */
	public calculateWindDirectionHeading(
		vesselHeading: number,
	 ...
	 */
	public calculateWindDirectionMagnetic(
	 ...
	 */
	public convertWindSpeed(
		speedMs: number,
		targetUnit: "kmh" | "knots" | "mph",
	): number {
		switch (targetUnit) {
			case "kmh":
