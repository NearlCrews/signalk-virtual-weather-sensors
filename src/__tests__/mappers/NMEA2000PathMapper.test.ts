...
describe("NMEA2000PathMapper", () => {
	...
	describe("NMEA2000 Compatibility", () => {
		...
		it("should provide temperature instance mappings aligned with sk-n2k-emitter", () => {
			...
			expect(
				instanceMap["environment.outside.apparentWindChillTemperature"],
			).toBe(103);
			...
			expect(instanceMap["environment.outside.wetBulbGlobeTemperature"]).toBe(
				111,
			);
		});
		...
		it("should sanitize data for NMEA2000 compatibility", () => {
			...
			const tempValue = values.find(
				(v) => v.path === "environment.outside.temperature",
			)?.value as number;
			const pressureValue = values.find(
				(v) => v.path === "environment.outside.pressure",
			)?.value as number;
			const humidityValue = values.find(
				(v) => v.path === "environment.outside.relativeHumidity",
			)?.value as number;
			const windSpeedValue = values.find(
	...
	describe("Data Validation", () => {
		...
		it("should handle warnings for out-of-range values", () => {
			...
			const isValid = mapper.validateWeatherDataForMapping(
				weatherDataWithWarnings,
			);
	...
	describe("Apparent Wind Integration", () => {
		it("should include calculated apparent wind when available", () => {
			...
			const speedValue = values.find(
				(v) => v.path === "environment.wind.speedApparent",
			)?.value;
			const angleValue = values.find(
				(v) => v.path === "environment.wind.angleApparent",
			)?.value;
	...
	describe("Marine Safety Features", () => {
		it("should map marine safety indices correctly", () => {
			...
			expect(
				values.find((v) => v.path === "environment.outside.heatStressIndex")
					?.value,
			).toBe(3);
			expect(
				values.find((v) => v.path === "environment.wind.beaufortScale")?.value,
			).toBe(8);
			expect(
				values.find((v) => v.path === "environment.outside.uvIndex")?.value,
			).toBe(9.5);
			expect(
				values.find((v) => v.path === "environment.outside.visibility")?.value,
			).toBe(500);
		});
	});
