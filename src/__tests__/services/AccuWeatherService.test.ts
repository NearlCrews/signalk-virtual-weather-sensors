...
describe("AccuWeatherService", () => {
	...
	describe("Constructor", () => {
		...
		it("should throw error for empty API key", () => {
			expect(() => new AccuWeatherService("")).toThrow(
		...
		it("should initialize logger and config correctly", () => {
			...
			const _customService = new AccuWeatherService(
				"test-key",
				customLogger,
				customConfig,
			);
	...
	describe("fetchCurrentWeather", () => {
		...
		it.skip("should handle API errors gracefully", async () => {
			...
			await expect(
				testService.fetchCurrentWeather(testLocation),
		...
		it("should validate location coordinates", async () => {
			...
			await expect(
				service.fetchCurrentWeather(invalidLocation),
	...
	describe("Data Quality Assessment", () => {
		...
		it("should increase quality for rich data sets", async () => {
			...
			const richResponse = createMockAccuWeatherResponse({
				WindGust: {
					Speed: {
						Metric: { Value: 30, Unit: "km/h" },
						Imperial: { Value: 18.6, Unit: "mi/h" },
					},
				},
				Visibility: {
					Metric: { Value: 20, Unit: "km" },
					Imperial: { Value: 12, Unit: "mi" },
				},
			});
			...
			(global.fetch as Mock)
				...
				.mockResolvedValueOnce({
					ok: true,
					json: () => Promise.resolve(richResponse),
				});
