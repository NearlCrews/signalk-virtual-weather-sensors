...
describe("WindCalculator", () => {
	...
	describe("Dew Point Calculation", () => {
		...
		it("should handle extreme humidity values", () => {
			...
			const highHumidityResult = calculator.calculateDewPoint(
				temperature,
				0.95,
			);
	...
	describe("Wind Direction Utilities", () => {
		...
		it("should convert wind direction to degrees correctly", () => {
			...
			expect(
				calculator.convertWindDirection((3 * Math.PI) / 2, "degrees"),
			).toBe(270); // West
		});
		...
		it("should convert wind direction to compass correctly", () => {
			...
			expect(
				calculator.convertWindDirection((3 * Math.PI) / 2, "compass"),
			).toBe("W");
			expect(calculator.convertWindDirection(Math.PI / 4, "compass")).toBe(
				"NE",
			);
		});
		...
		it("should calculate relative wind direction", () => {
			...
			const result = calculator.calculateRelativeWindDirection(
				windDirection,
				vesselHeading,
			);
	...
	describe("Wind Speed Conversions", () => {
		...
		it("should convert wind speed to different units", () => {
			...
			expect(calculator.convertWindSpeed(windSpeedMs, "kmh")).toBeCloseTo(
				36,
				1,
			); // km/h
			expect(calculator.convertWindSpeed(windSpeedMs, "knots")).toBeCloseTo(
				19.44,
				1,
			); // knots
			expect(calculator.convertWindSpeed(windSpeedMs, "mph")).toBeCloseTo(
				22.37,
				1,
			); // mph
		});
	...
	describe("Input Validation", () => {
		it("should validate all required inputs", () => {
			...
			// Test invalid inputs
			expect(calculator.validateWindInputs(Number.NaN, 5, 0, Math.PI / 2)).toBe(
				false,
			);
			expect(calculator.validateWindInputs(10, -1, 0, Math.PI / 2)).toBe(false);
			expect(calculator.validateWindInputs(10, 1000, 0, Math.PI / 2)).toBe(
				false,
			);
		...
		it("should normalize angles correctly", () => {
			...
			expect(Math.abs(calculator.normalizeAngle(Math.PI * 3))).toBeCloseTo(
				Math.PI,
				2,
			);
			expect(Math.abs(calculator.normalizeAngle(-Math.PI * 3))).toBeCloseTo(
			...
			expect(calculator.normalizeAngle(Math.PI / 2)).toBeCloseTo(
				Math.PI / 2,
				2,
			);
		});
	...
	describe("Performance and Precision", () => {
		...
		it("should maintain precision across multiple calculations", () => {
			...
			expect(results.every((r) => Math.abs(r - firstResult) < 0.0001)).toBe(
				true,
			);
		});
	});
