...
export class NMEA2000PathMapper {
    ...
    if (data.apparentTemperature !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'AccuWeather apparent temperature (different from RealFeel)',
        },
      });
    }
  }
   ...
   * @private
   */
  private addHumidityPaths(values: SignalKValue[], data: WeatherData, timestamp: string): void {
    ...
    if (data.absoluteHumidity !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'Calculated absolute humidity from temperature and relative humidity',
        },
      });
    }
  }
   ...
   * @private
   */
  private addWindPaths(values: SignalKValue[], data: WeatherData, timestamp: string): void {
    ...
    if (data.beaufortScale !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'Beaufort scale (0-12) calculated from wind and gust data',
        },
      });
    }
    ...
    if (data.apparentWindSpeed !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'Calculated apparent wind speed relative to vessel motion',
        },
      });
    }
   ...
   * @private
   */
  private addAtmosphericPaths(values: SignalKValue[], data: WeatherData, timestamp: string): void {
   ...
   * @private
   */
  private addCalculatedPaths(values: SignalKValue[], data: WeatherData, timestamp: string): void {
    ...
    if (data.airDensityEnhanced !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'Enhanced air density calculation including humidity and pressure effects',
        },
      });
    }
  }
   ...
   * @private
   */
  private addSafetyPaths(values: SignalKValue[], data: WeatherData, timestamp: string): void {
    ...
    if (data.heatStressIndex !== undefined) {
      values.push({
        ...
        meta: {
          ...
          description: 'Heat stress assessment (0=none, 4=extreme) from wet bulb globe temperature',
        },
      });
    }
  ...
  public getTemperatureInstanceMap(): Record<string, number> {
    return {
      ...
      'environment.outside.dewPointTemperature': TEMPERATURE_INSTANCES.DEW_POINT,
      'environment.outside.windChillTemperature': TEMPERATURE_INSTANCES.WIND_CHILL,
      'environment.outside.heatIndexTemperature': TEMPERATURE_INSTANCES.HEAT_INDEX,
      'environment.outside.realFeelShade': TEMPERATURE_INSTANCES.REAL_FEEL_SHADE,
      ...
      'environment.outside.wetBulbGlobeTemperature': TEMPERATURE_INSTANCES.WET_BULB_GLOBE,
    };
  }
   ...
   * @private
   */
  private countPathsByCategory(values: SignalKValue[], category: string): number {
  ...
  public getPathStatistics(values: SignalKValue[]): {
    ...
    return {
      ...
      calculated: values.filter(
        (v) =>
          v.meta?.description?.includes('calculated') || v.meta?.description?.includes('Calculated')
      ).length,
